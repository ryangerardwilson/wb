...
write_public_launcher() {
  ...
  elif [[ -f "$PUBLIC_LAUNCHER" ]] && ! grep -Eq '^# Managed by [[:alnum:]_-]+ local-bin launcher$' "$PUBLIC_LAUNCHER" 2>/dev/null; then
...
#!/usr/bin/env bash
# Managed by wb local-bin launcher
set -euo pipefail
